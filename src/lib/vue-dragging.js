...
var _ = {
    ...
    removeClass: function removeClass(el, cls) {
        if (arguments.length < 2) {
            ...
            for (var i = 1, len = arguments.length; i < len; i++) {
                el.classList && el.classList.remove(arguments[i]);
            }
        }
    }
};
...
var vueDragging = function (Vue, options) {
    ...
    function handleDragStart(e) {
        ...
        if (e.dataTransfer) {
            ...
            console.log(e.pageX, el.clientLeft, el.clientHeight)
    ...
    function handleDragEnd(e) {
        e.stopPropagation();
        var el = getBlockEl(e.target) || this.el;
        if (!el) return
        ...
        $dragging.$emit('dragend', { group: group, list: DDD.List });

        var dragend = el.getAttribute('dragend');
        // console.log(this, 'handleDragEndhandleDragEndhandleDragEnd')
        let vm = this.context || this.vm
        if (dragend && vm && vm[dragend]) {
            // 防抖，防止在短时间内调用多次
            this._debounce || (this._debounce = debounce(vm[dragend], 500))
            this._debounce()
        }
    }
    ...
    function addDragItem(el, binding, vnode) {
        ...
        let el2 = el.querySelector(target) || el
        ...
        el.setAttribute('dragend', binding.value.dragend);
