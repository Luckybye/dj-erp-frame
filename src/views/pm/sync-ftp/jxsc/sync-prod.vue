<template lang="html">
  <div class="sync-prod-data">
    <div class="mb15">
      <el-button type="primary" @click="onSync">新建同步</el-button>
    ...
    <div class="flex">
      ...
      <div class="flex-1">
        ...
        <x-table :datas="logDetails" :page="searchModel" :getData="refresh">
          <x-table-column class="text-center" label="同步路径">
            <div slot-scope="{row}">{{row.file_name}}</div>
          </x-table-column>
          <x-table-column class="text-center" label="商品名">
            <div slot-scope="{row}">
              {{row.prod_name}}
...
export default {
  ...
  methods: {
    ...
    onSync () {
      ...
      this.$get('/api/manage/createProdSynPlan').then((data) => {
        this.queryLogs().then(() => {
          this.currentIndex = 0
          this.timeRefresh()
        })
    ...
    refresh (i) {
      ...
      return this.$get('/api/manage/queryProdSynDetail', {
        syn_plan_id: id,
        ...search
      }._trim(), {loading: true}).then((data) => {
        this.logDetails = data.prod_syn_details || {}
        return data
      })
    },
    queryLogs () {
      return this.$get('/api/manage/queryProdSynPlan', {file_type: 'excel'}).then(data => {
