<template>
  <div>
    ...
    <x-fold class="mb10" show>
      ...
      <div>
        <el-checkbox v-model="vm.sc_config.check_port_before_submit" @change="onSave('sc_config')" true-label="no" false-label="tip"
        >不约束</el-checkbox>
        <el-checkbox v-model="vm.sc_config.check_port_before_submit" @change="onSave('sc_config')" true-label="tip" false-label="no" class="ml20"
        >起运港不同时提醒</el-checkbox>
        <el-checkbox v-model="vm.sc_config.check_port_before_submit" @change="onSave('sc_config')" true-label="disabled" false-label="no" class="ml20"
...
export default {
  ...
  data() {
    return {
      scRules: [
        // { result: "sc_config", field: "lock_mycom3", text: "我方法人公司是否锁定", type: "checkbox" },
        // { result: "sc_config", field: "select_catalog2", text: "产品可否来源于产品档案", type: "checkbox" },
        { result: "sc_config", field: "select_sameprod", text: "是否可选同一商品货号的商品", type: "checkbox" },
        { result: "sc_config", field: "fill_up", text: "产品数量是否整箱处理", type: "checkbox" },
        { result: "sc_config", field: "shipping_day", text: "订单默认客户交期", type: "input" },
        { result: "sc_config", field: "delivery_day", text: "交货提前期", type: "input" },
        { result: "sc_config", field: "guidance_price", text: "产品价格能否低于指导价", type: "checkbox" },
        { result: "busi_rules", field: "update_cust_price", text: "订单生效后更新客户专属价格", type: "checkbox" },
      ],
      vm: {
        sc_config: {
          fill_up: "yes",
