    ...
    <el-table :data="datas" tooltip-effect="dark" style="width: 100%">
      <el-table-column :label="$t('enable_index')">
        <template slot-scope="scope">
          <el-checkbox v-model="scope.row.isSelected" @change="onSave()" :disabled="scope.row.require">{{
            scope.$index + 1
          }}</el-checkbox>
        </template>
      </el-table-column>
      <el-table-column :label="$t('busi_point')">
        <template slot-scope="scope">{{ scope.row.value || "-" }}
          <span v-if="scope.row.require" class="text-red">（必选）</span>
        </template>
      </el-table-column>
...
<script>
import Timeline from '@/lib/timeline/bk'
export default {
  data() {
    return {
