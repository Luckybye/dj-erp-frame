<template>
  <div class="x-component search-select-ec-company" :style="{width: width}">
    <x-select
      width="100%"
      :source="datas"
      :multiple="multiple"
      :placeholder="placeholder || label"
      :map="{ value: 'ec_com_id', label: 'ec_com_name', disabled: 'x_disabled' }"
      v-model="vmodel"
      :clearable="clearable"
...
export default {
  ...
  methods: {
    ...
    async getDatas () {
      ...
      let v = await this.$get('/ideal/wf/queryEcCompanys', { platform_id }, {loading: false, cache: 2})
      this.preDatas = v.ec_companys || []
      this.preDatas.map(item => {
        item.filter = item.ec_com_name + item.ec_com_code
      })
    },
    async getPlatform() {
