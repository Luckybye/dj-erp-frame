<template>
  <div class="x-component search-select-our-com" :style="{width: width}">
    <x-select
      width="100%"
      :source="datas"
      :multiple="multiple"
      :placeholder="placeholder || label || $t('all')"
      :map="{ value: 'id', label: $i18n.locale === 'cn' ? 'text' : 'text_en', disabled: 'x_disabled' }"
      v-model="vmodel"
      :clearable="clearable"
...
export default {
  ...
  methods: {
    ...
    async getDatas () {
      this.preDatas = await this.$cache.getMyCompanyLegal()
      this.preDatas.sort((a, b) => b.x_disabled ? -1 : 1)
    }
  },
  computed: {
    vmodel: {
