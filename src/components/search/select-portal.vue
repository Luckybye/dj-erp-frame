<template>
  <div class="x-component search-select-portal" :style="{width: width}">
    <x-select
      width="100%"
      :source="datas"
      :multiple="multiple"
      :placeholder="placeholder || label"
      :map="{ value: 'cm_portal_id', label: 'portal_name', disabled: 'x_disabled' }"
      v-model="vmodel"
      :clearable="clearable"
...
export default {
  ...
  methods: {
    ...
    async getDatas () {
      this.preDatas = (await this.$get('/api/crm/queryCmPortals')).cm_portals || []
    }
  },
  computed: {
    vmodel: {
