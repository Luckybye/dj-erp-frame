<template>
  <div class="x-component search-select-remittance" :style="{width: width}">
    <x-select
      width="100%"
      :source="datas"
      :multiple="multiple"
...
export default {
  ...
  methods: {
    ...
    async getDatas () {
      // this.preDatas = ((await this.$configure.getValue('remittance_type', this.$state('me').com_id)).remittance_type || []).map(m => {
