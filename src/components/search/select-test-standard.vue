<template>
  <div class="x-component search-select-test-standard" :style="{width: width}">
    <x-select
      width="100%"
      :source="datas"
      :multiple="multiple"
      :placeholder="label"
      :map="{ value: 'standard', label: 'standard' }"
      v-model="vmodel"
      :clearable="clearable"
...
export default {
  ...
  methods: {
    ...
    async getDatas () {
      this.preDatas = (await this.$configure.getValue('test_standard', this.$state('me').com_id)).test_standard
    }
  },
  computed: {
    vmodel: {
