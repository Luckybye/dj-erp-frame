<template>
  <div class="x-component search-select-brand" :style="{width: width}">
    <x-select
      width="100%"
      :source="datas"
      :multiple="multiple"
      :placeholder="placeholder || label || $t('all_brand')"
      :map="{ value: 'brand_id', label: tfield('brand_name'), disabled: 'x_disabled' }"
      v-model="vmodel"
      :clearable="clearable"
...
export default {
  ...
  methods: {
    ...
    async getDatas () {
      this.preDatas = await this.$cache.getBrands()
    }
  },
  computed: {
    vmodel: {
