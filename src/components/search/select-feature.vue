<template>
  <div class="x-component search-feature" :style="{width: width}">
    <x-select
      width="100%"
      v-model="vmodel"
      :source="datas"
...
export default {
  ...
  methods: {
    ...
    async getDatas () {
      let m = this.selected._object('type')
      this.datas = this.$constant("prodFeature").filter(f => !m[f.key] || f.key === this.vmodel)
    },
    getLabel () {
