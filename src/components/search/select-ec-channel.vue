<template>
  <div class="x-component search-select-ec-channel" :style="{width: width}">
    <x-select
      width="100%"
      :source="datas"
      :multiple="multiple"
      :placeholder="placeholder || label"
      :map="{ value: 'ec_channel_id', label: 'ec_channel_name', disabled: 'x_disabled' }"
      v-model="vmodel"
      :clearable="clearable"
...
export default {
  ...
  methods: {
    ...
    async getDatas () {
      let platform_id = await this.getPlatform()
      if (!platform_id) return
      let v = await this.$get('/ideal/wf/queryEcChannels', { platform_id }, {loading: false, cache: 2})
      this.preDatas = v.ec_channels || []
      this.preDatas.map(item => {
        item.filter = item.ec_channel_name + item.ec_channel_code
      })
    },
    async getPlatform() {
      ...
      let platform_id = ''
      v && v.forEach(item => {
        if (item.platform_code === 'sellercloud') {
